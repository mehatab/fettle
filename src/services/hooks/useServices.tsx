...
async function logs(key: string): Promise<LogDaySummary[]> {
  ...
  const prepareSummary = Object.values(
    logs.reduce((r: any, date) => {
  ...
  prepareSummary.forEach((logSummary: any) => {
    ...
    if (logSummary.logs.length === 0) {
      ...
      logSummary.logs.every((item: any) => item.status === "success")
    ) {
...
function fillData(data: LogDaySummary[]): LogDaySummary[] {
  ...
  for (var i = -1; i < 89; i += 1) {
    ...
    const summary = data.find(
      (item) => item.date === d.toISOString().substr(0, 10)
    );
    logDaySummary.push({
